  ...
  if (req.method === "GET") {
    ...
    let VoteSlideInput = () => {
      ...
      function getSteemPower(params) {
        ...
        let rshares = parseInt(
          account_vesting_shares_available * used_power * 100
        );
        rshares =
          ((rshares + 2e12) * (rshares + 2e12) - 4e24) / (rshares + 8e12);
      ...
      function finalize(rshares_convergent_linear, steem_per_rshares) {
        steem.api.getCurrentMedianHistoryPrice(function (e, t) {
          let steem_price_sbd =
            t.base.replace(/ SBD| HBD/, "") /
            t.quote.replace(/ STEEM| HIVE/, "");
